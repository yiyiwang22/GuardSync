...
const Map<String, Map<String, String>> metricInfo = {
  ...
  'bodyTemp': { 'title': 'Body Temperature', 'measures': 'Core body temperature in degrees Fahrenheit.', 'colorMeans': 'Green indicates normal temperature.\nYellow indicates slightly elevated.\nOrange indicates fever/high exertion.\nRed indicates risk of heat illness.', 'acceptableRanges': 'Green 97-99.5°F, Yellow 99.6-99.9°F, Orange 100-100.4°F, Red > 100.4°F.', 'standard': 'Uses clinical body temperature guidelines for athletic activity.' },
  'biteForce': { 'title': 'Bite Force', 'measures': 'Pressure exerted when clenching jaw, measured in Newtons.', 'colorMeans': 'Green indicates normal jaw tension.\nYellow indicates mild clenching.\nOrange indicates significant stress.\nRed indicates risk of dental or jaw damage.', 'acceptableRanges': 'Green < 200N, Yellow 200-250N, Orange 250-300N, Red > 300N.', 'standard': 'Uses biomechanical jaw force standards.' },
};
      ...
      return AlertDialog(
        ...
        title: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            IconButton(icon: const Icon(Icons.close, color: Colors.grey), onPressed: () => Navigator.of(context).pop(), padding: EdgeInsets.zero, constraints: const BoxConstraints()),
          ]),
          const SizedBox(height: 12),
          Text(info['title']!, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF1A1A1A), height: 1.2)),
        ]),
        content: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
          ...
          SizedBox(width: double.infinity, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1A5C4C), foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), onPressed: () => Navigator.of(context).pop(), child: const Text('Got it', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)))),
        ])),
      );
    });
  }
    ...
    return Scaffold(
      body: SafeArea(
        child: Column(children: [
          ...
          Padding(padding: const EdgeInsets.all(20.0), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Row(children: [
              Container(width: 44, height: 44, decoration: const BoxDecoration(color: Color(0xFFD4D2C5), shape: BoxShape.circle), child: const Icon(Icons.person, color: Color(0xFF666666))),
              const SizedBox(width: 12),
              Text(_service.name, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1A1A1A))),
            ]),
            Row(children: [
              GestureDetector(onTap: () => Navigator.pushNamed(context, '/athlete-connections'), child: Container(width: 36, height: 36, decoration: const BoxDecoration(color: Color(0xFFD4D2C5), shape: BoxShape.circle), child: const Icon(Icons.link, color: Color(0xFF1A5C4C), size: 18))),
              const SizedBox(width: 10),
              Container(width: 36, height: 36, decoration: const BoxDecoration(color: Color(0xFFD4D2C5), shape: BoxShape.circle), child: const Icon(Icons.bluetooth, color: Color(0xFF1A5C4C), size: 18)),
            ]),
          ])),
          // Original Profile Info Bar
          Container(
            margin: const EdgeInsets.symmetric(horizontal: 16),
            padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 14),
            decoration: BoxDecoration(color: const Color(0xFFD4D2C5), borderRadius: BorderRadius.circular(16)),
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                RichText(text: const TextSpan(text: 'Status: ', style: TextStyle(color: Color(0xFF555555), fontSize: 14), children: [TextSpan(text: 'Connected', style: TextStyle(color: Color(0xFF1A5C4C), fontWeight: FontWeight.bold))])),
                Text('Sex: ${_service.sex}', style: const TextStyle(color: Color(0xFF555555), fontSize: 14)),
              ]),
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Weight: ${_service.weight}', style: const TextStyle(color: Color(0xFF555555), fontSize: 14)),
                Text('Height: ${_service.height}', style: const TextStyle(color: Color(0xFF555555), fontSize: 14)),
              ]),
            ]),
          ),
          const SizedBox(height: 16),
      ...
      bottomNavigationBar: Container(
        ...
        child: SafeArea(child: Row(children: [
          Expanded(child: GestureDetector(onTap: () {}, child: Container(height: 48, decoration: BoxDecoration(color: const Color(0xFF1A5C4C), borderRadius: BorderRadius.circular(12)), child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.dashboard, color: Colors.white, size: 20), SizedBox(width: 8), Text('Dashboard', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))])))),
          const SizedBox(width: 12),
          Expanded(child: GestureDetector(onTap: () => Navigator.pushNamed(context, '/trainer-analytics'), child: Container(height: 48, decoration: BoxDecoration(color: const Color(0xFFF5F5F0), borderRadius: BorderRadius.circular(12)), child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.trending_up, color: Color(0xFF555555), size: 20), SizedBox(width: 8), Text('History', style: TextStyle(color: Color(0xFF555555), fontWeight: FontWeight.bold))])))),
        ])),
      ),
    );
  }

  Widget _buildMetricCard(String label, String value, dynamic icon, Color valueColor, String? infoKey) {
    return Container(
      ...
      child: Stack(children: [
        if (infoKey != null) Positioned(top: 4, right: 4, child: IconButton(icon: const Icon(Icons.info_outline, color: Color(0xFF999999), size: 18), onPressed: () => _showInfoDialog(infoKey))),
        Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 4),
          AnimatedDefaultTextStyle(duration: const Duration(milliseconds: 300), style: TextStyle(color: valueColor, fontSize: 20, fontWeight: FontWeight.bold), child: Text(value)),
        ])),
      ]),
    );
  }
...
class _ToothPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path = Path();
    path.moveTo(8, 8); path.cubicTo(8, 2, 16, 2, 16, 6); path.cubicTo(16, 2, 24, 2, 24, 8);
    path.cubicTo(24, 16, 28, 18, 24, 26); path.cubicTo(22, 32, 18, 30, 16, 22);
    path.cubicTo(14, 30, 10, 32, 8, 26); path.cubicTo(4, 18, 8, 16, 8, 8);
