...
class _DashboardPageState extends State<DashboardPage> {
  ...
  void _startSimulation() {
    _timer = Timer.periodic(const Duration(seconds: 2), (timer) {
      setState(() {
        double hrZone = _random.nextDouble();
        if (hrZone < 0.4) {
        ...
        double spo2Zone = _random.nextDouble();
        if (spo2Zone < 0.4) {
          spO2 = 95 + _random.nextInt(6);
        } else if (spo2Zone < 0.6) spO2 = 93 + _random.nextInt(2);
        else if (spo2Zone < 0.8) spO2 = 92;
        else spO2 = 88 + _random.nextInt(4);

        double tempZone = _random.nextDouble();
        if (tempZone < 0.4) {
          bodyTemp = double.parse((97 + _random.nextDouble() * 2.5).toStringAsFixed(1));
        } else if (tempZone < 0.6) bodyTemp = double.parse((99.6 + _random.nextDouble() * 0.4).toStringAsFixed(1));
        else if (tempZone < 0.8) bodyTemp = double.parse((100 + _random.nextDouble() * 0.5).toStringAsFixed(1));
        else bodyTemp = double.parse((100.5 + _random.nextDouble() * 1.5).toStringAsFixed(1));

        double biteZone = _random.nextDouble();
        if (biteZone < 0.4) {
          biteForce = 120 + _random.nextInt(80);
        } else if (biteZone < 0.6) biteForce = 200 + _random.nextInt(51);
        else if (biteZone < 0.8) biteForce = 250 + _random.nextInt(51);
        else biteForce = 301 + _random.nextInt(100);
      });
    });
  }

  Color getHeartRateColor(int hr) {
    if (hr >= 190) return const Color(0xFFE63946); // Red
    if (hr >= 165) return const Color(0xFFF77F00); // Orange
    if (hr >= 150) return const Color(0xFFFBBF24); // Yellow
    return const Color(0xFF1A5C4C); // Green
  }
          ...
          children: [
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const Text('John Smith', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1A1A1A))),
                    ],
                  ),
                  Container(
                    width: 36, height: 36,
                    decoration: const BoxDecoration(color: Color(0xFFD4D2C5), shape: BoxShape.circle),
                    child: const Icon(Icons.bluetooth, color: Color(0xFF1A5C4C), size: 18),
                  ),
                ],
              ),
            ),
                  ...
                  Column(
                    ...
                    children: [
                      RichText(text: const TextSpan(
                        text: 'Status: ', style: TextStyle(color: Color(0xFF555555), fontSize: 14),
                        children: [TextSpan(text: 'Connected', style: TextStyle(color: Color(0xFF1A5C4C), fontWeight: FontWeight.bold))],
                      )),
                      const Text('Sex: Male', style: TextStyle(color: Color(0xFF555555), fontSize: 14)),
                    ],
                  ),
                  const Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Weight: 247 lbs', style: TextStyle(color: Color(0xFF555555), fontSize: 14)),
                      Text("Height: 6' 2''", style: TextStyle(color: Color(0xFF555555), fontSize: 14)),
                    ],
                  ),
                ],
              ),
            ...
            Expanded(
              child: GridView.count(
                ...
                children: [
                  _buildMetricCard('Sport', 'Football', Icons.directions_run, const Color(0xFF1A5C4C), null),
                  _buildMetricCard('Active Minutes', '1H 21M', Icons.hourglass_empty, const Color(0xFF1A5C4C), null),
                  _buildMetricCard('Head Acceleration', '95g', Icons.sports_motorsports, const Color(0xFF1A5C4C), null),
                  _buildMetricCard('Concussion Risk', 'High', Icons.warning_amber_rounded, const Color(0xFFE63946), null),
                  _buildMetricCard('Heart Rate', '$heartRate BPM', Icons.favorite, getHeartRateColor(heartRate), 'heartRate'),
                 // _buildMetricCard('SpO2', '$spO2%', Icons.air, getSpO2Color(spO2), 'spO2'),
                  _buildMetricCard('SpO2', '$spO2%', O2Icon(color: getSpO2Color(spO2)), getSpO2Color(spO2), 'spO2'),
                  _buildMetricCard('Body Temperature', '$bodyTemp °F', Icons.thermostat, getTempColor(bodyTemp), 'bodyTemp'),
                 // _buildMetricCard('Bite Force', '$biteForce N', Icons.compress, getBiteForceColor(biteForce), 'biteForce'),
                 _buildMetricCard('Bite Force', '$biteForce N', ToothIcon(color: getBiteForceColor(biteForce)), getBiteForceColor(biteForce), 'biteForce'),
                ],
              ),
            ),
          ],
          ...
          child: Row(
            children: [
              Expanded(
                child: Container(
                  height: 48,
                  decoration: BoxDecoration(color: const Color(0xFF1A5C4C), borderRadius: BorderRadius.circular(12)),
                  child: const Row(
              ...
              Expanded(
                child: Container(
                  ...
                  child: const Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(Icons.trending_up, color: Color(0xFF555555), size: 20),
                      SizedBox(width: 8),
                      Text('History', style: TextStyle(color: Color(0xFF555555), fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
              ),
...
class O2Icon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return RichText(
      text: TextSpan(
        text: 'O',
        style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: color, height: 1.1),
        children: [
          WidgetSpan(
            child: Transform.translate(
              offset: const Offset(1, 6), 
              child: Text('2', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: color)),
            ),
          ),
        ],
      ),
